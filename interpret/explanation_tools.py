...
def sensitivity_analysis_cat(BNN, dset, aleatoric_coeff, epistemic_coeff, batch_size=1024, cuda=True):
    """Returns mean of gradient absoluteb values at each input"""
    if cuda:
        trainloader = torch.utils.data.DataLoader(dset, batch_size=batch_size, shuffle=True, pin_memory=True, num_workers=3)
    ...
    for x, y in trainloader:
        ...
        objective = aleatoric_coeff * aleatoric_entropy.sum() + epistemic_coeff * epistemic_entropy.sum()
        objective.backward()
...
def sensitivity_analysis_gauss(BNN, dset, aleatoric_coeff, epistemic_coeff, batch_size=1024, cuda=True, entropy=False):
    """Returns mean of gradient absoluteb values at each input"""
    if cuda:
        trainloader = torch.utils.data.DataLoader(dset, batch_size=batch_size, shuffle=True, pin_memory=True, num_workers=3)
...
def evaluate_aleatoric_std(VAEAC, explanations, test_dims, N_target_samples=500):
    ...
    y_cond_explanations_mu, y_cond_explanations_std = \
        sample_artificial_targets_gauss(VAEAC, explanations_expand, test_dims, N_target_samples)
...
def evaluate_epistemic_rms(BNN, VAEAC, explanations, test_dims, nsamples=500):
    """This assumes that test dims are placed at end of input vector"""
    explanations_expand = torch.cat([explanations, explanations.new_zeros((explanations.shape[0],
                                                                           len(test_dims)))], dim=1)
    y_cond_explanations_mu_epistemic, _ = \
        sample_artificial_targets_gauss(VAEAC, explanations_expand,
                                        test_dims, N_target_samples=nsamples, z_mean=False)

    y_cond_explanations_mu_epistemic = y_cond_explanations_mu_epistemic.mean(dim=0)  # we have 1 sample

    sensitivity_rms, x_all_abs_error = evaluate_BNN_epistemic_rms(BNN, explanations, y_cond_explanations_mu_epistemic)
