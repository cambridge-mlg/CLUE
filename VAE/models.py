...
class MNIST_recognition_resnet(nn.Module):
    def __init__(self, latent_dim):
        ...
        proposal_layers = [nn.Conv2d(1, 32, kernel_size=1, padding=0, stride=1),  # 28x28 --32
                           ...
                           nn.Conv2d(128, 128, kernel_size=3, padding=1, stride=2), nn.LeakyReLU(inplace=True),  # 4x4 --128
                           small_MNIST_Flatten(), nn.BatchNorm1d(num_features=4 * 4 * 128), nn.Linear(4*4*128, latent_dim*2),
                           SkipConnection(
                               nn.LeakyReLU(),
...
class MNIST_generator_resnet(nn.Module):
    def __init__(self, latent_dim):
        ...
        generative_layers = [leaky_MLPBlock(latent_dim),
                             nn.Linear(latent_dim, 4*4*128), nn.LeakyReLU(), nn.BatchNorm1d(num_features=4*4*128), small_MNIST_unFlatten(),  # 4x4 --128
...
class small_MNIST_recognition_net(nn.Module):
    def __init__(self, latent_dim):
        ...
        proposal_layers = [nn.Conv2d(1, self.Nfilt1, kernel_size=4, padding=1, stride=2), nn.ReLU(inplace=True), nn.BatchNorm2d(num_features=self.Nfilt1),
                           nn.Conv2d(self.Nfilt1, self.Nfilt2, kernel_size=4, padding=1, stride=2), nn.ReLU(inplace=True),  nn.BatchNorm2d(num_features=self.Nfilt2),
                           nn.Conv2d(self.Nfilt2, self.Nfilt3, kernel_size=3, padding=1, stride=2), nn.ReLU(inplace=True), small_MNIST_Flatten(),
                           nn.BatchNorm1d(num_features=4 * 4 * self.Nfilt3), nn.Linear(4 * 4 * self.Nfilt3, latent_dim*2)]
...
class small_MNIST_generator_net(nn.Module):
    def __init__(self, latent_dim):
        ...
        self.Nfilt3 = 128
        # input layer
        generative_layers = [nn.Linear(latent_dim, 4 * 4 * self.Nfilt3), nn.ReLU(inplace=True), nn.BatchNorm1d(num_features=4 * 4 * self.Nfilt3),
                             small_MNIST_unFlatten(),
                             nn.ConvTranspose2d(self.Nfilt3, self.Nfilt2, kernel_size=3, padding=1, stride=2), nn.ReLU(inplace=True), nn.BatchNorm2d(num_features=self.Nfilt3),
                             nn.ConvTranspose2d(self.Nfilt2,   self.Nfilt1, kernel_size=4, padding=1, stride=2), nn.ReLU(inplace=True), nn.BatchNorm2d(num_features=self.Nfilt1),
...
class doodle_recognition_resnet(nn.Module):
    def __init__(self, latent_dim):
        ...
        proposal_layers = [nn.Conv2d(1, 32, kernel_size=1, padding=0, stride=1),  # 64x64 --32
                           ResBlock(outer_dim=32, inner_dim=8*width_mul), ResBlock(outer_dim=32, inner_dim=8*width_mul),
                           ...
                           ResBlock(outer_dim=128, inner_dim=32*width_mul), ResBlock(outer_dim=128, inner_dim=32*width_mul),
                           nn.Conv2d(128, 128, kernel_size=4, padding=1, stride=2), nn.LeakyReLU(inplace=True),  # 4x4 --128
                           small_MNIST_Flatten(), nn.BatchNorm1d(num_features=4 * 4 * 128), nn.Linear(4*4*128, latent_dim*2),
                           SkipConnection(
                               nn.LeakyReLU(),
                               nn.BatchNorm1d(num_features=latent_dim*2),
...
class doodle_generator_resnet(nn.Module):
    def __init__(self, latent_dim):
        ...
        generative_layers = [leaky_MLPBlock(latent_dim),
                             nn.Linear(latent_dim, 4*4*128), nn.LeakyReLU(inplace=True), nn.BatchNorm1d(num_features=4*4*128), small_MNIST_unFlatten(),  # 4x4 --128
                             nn.ConvTranspose2d(128, 128, kernel_size=4, padding=1, stride=2),  # 8x8 --128
                             ResBlock(outer_dim=128, inner_dim=32*width_mul), ResBlock(outer_dim=128, inner_dim=32*width_mul),
                             nn.ConvTranspose2d(128, 128, kernel_size=4, padding=1, stride=2),  # 16x16 --128
                             ResBlock(outer_dim=128, inner_dim=32*width_mul), ResBlock(outer_dim=128, inner_dim=32*width_mul),
                             nn.ConvTranspose2d(128, 64, kernel_size=4, padding=1, stride=2),  # 32x32 --64
                             ResBlock(outer_dim=64, inner_dim=16*width_mul), ResBlock(outer_dim=64, inner_dim=16*width_mul),
                             nn.ConvTranspose2d(64, 32, kernel_size=4, padding=1, stride=2),  # 64x64 --32
                             ResBlock(outer_dim=32, inner_dim=8*width_mul), ResBlock(outer_dim=32, inner_dim=8*width_mul),
