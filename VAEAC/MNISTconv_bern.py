...
class VAEAC_bern_net(BaseNet):
    def __init__(self, input_dim, latent_dim, lr=1e-4, targets=False,
                 recognition_net=None, generator_net=None, prior_net=None, cuda=True):
    ...
    def create_net(self, recognition_net, generator_net, prior_net):
        ...
        self.model = MNIST_VAEAC_bern(self.latent_dim, targets=self.targets, recognition_net=recognition_net,
                                      generator_net=generator_net, prior_net=prior_net)
        if self.cuda:
