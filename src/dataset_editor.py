...
class dataset_editor():
    ...
    def random_train_subset(self, nkeep):
        ...
        chosen_idxs = choice(np.arange(len(self.full_trainset)), size=(nkeep), replace=False, p=None)
    ...
    def notebook_display(self, labels=None, nview=0, Nrows=4, Ncols=5, train=True, dpi=200, hspace=5):
        if train:
            data = self.full_trainset.data
        else:
            data = self.full_testset.data

        dataset_editor.notebook_display_s(data, labels=labels, nview=nview, Nrows=Nrows, Ncols=Ncols, dpi=dpi, hspace=hspace)

    @staticmethod
    def notebook_display_s(data, labels=None, nview=0, Nrows=4, Ncols=5, dpi=200, hspace=5):
    ...
    def get_KNN(self, samples, K, train=True, dist=None, mean=False):
        ...
        pairwise = dist(samples.unsqueeze(dim=1), data.unsqueeze(dim=0)).type(torch.FloatTensor)
