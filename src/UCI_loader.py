...
def load_UCI(dset_name, splits=10, seed=0, separate_targets=True, save_dir='data/'):
    ...
    if dset_name == 'boston':
        ...
        if not os.path.isfile(save_dir+'winequality-red.csv'):
            urllib.urlretrieve("https://archive.ics.uci.edu/ml/machine-learning-databases/wine-quality/winequality-red.csv",
        ...
        data = pd.read_excel(save_dir + 'default of credit card clients.xls', header=[0, 1], index_col=0,
                         delimiter="\s+").values
        y_idx = [-1]  # OK
